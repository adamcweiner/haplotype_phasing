...
def em_algorithm2(data):
	...
	for ind in range(m_ind):
		...
		if n_ones <= 0:
			pool_add(ind_geno1.copy(), ind, 0)
			pool_add(ind_geno2.copy(), ind, 0)
			continue
		...
		for idx, comb in enumerate(lst):
			if no_nequal_spot:
				...
				compl_lst = [1]*n_ones-comb_lst
			ind_geno1[unphased_spot] = comb_lst
			ind_geno2[unphased_spot] = compl_lst
			pool_add(ind_geno1.copy(), ind, idx)
			pool_add(ind_geno2.copy(), ind, idx)
	poolprob = poolprob / np.sum(poolprob)
	print('Pool size ', len(poolprob))
